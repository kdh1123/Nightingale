...
export function SystemStatusPage() {
  ...
  return (
    <section>
      <SectionHeader
        eyebrow={
          ko ? "시스템 원격 측정 · 2초마다 갱신" : "System telemetry · refreshed every 2 seconds"
        }
        title={ko ? "시스템 상태" : "System health"}
        action={<span className="severity low">● {ko ? "실시간" : "LIVE"}</span>}
      />
      <div className="page-grid">
          ...
          <article className="card panel">
            ...
            <div className="filters">
              <input
                ...
                placeholder={ko ? "이름 또는 PID" : "Name or PID"}
              />
              <select
                aria-label="프로세스 정렬"
                value={sort}
              ...
              >
                <option value="cpu">{ko ? "CPU 사용률순" : "Sort by CPU"}</option>
                <option value="memory">{ko ? "메모리 사용량순" : "Sort by memory"}</option>
                <option value="name">{ko ? "이름순" : "Sort by name"}</option>
              </select>
            </div>
            {processes.isError ? (
              <p className="empty">프로세스 목록을 불러올 수 없습니다.</p>
            ) : (
              <div>
                {(processes.data ?? []).slice(0, 12).map((item) => (
        ...
        <div className="stack">
          <article className="card panel">
            <h2>{ko ? "하드웨어 정보" : "Hardware information"}</h2>
